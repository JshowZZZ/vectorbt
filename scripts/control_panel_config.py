...
def try_handle_post(handler, parsed):
    ...
    if parsed.path == "/pause":
        _write_control(True)
        handler._send(json.dumps({"ok": True, "message": "已暫停回測。"}, ensure_ascii=False), "application/json; charset=utf-8")
        return True
    if parsed.path == "/resume":
        _write_control(False)
        handler._send(json.dumps({"ok": True, "message": "已繼續回測。"}, ensure_ascii=False), "application/json; charset=utf-8")
        return True
