...
def _start_run():
    ...
    with PROCESS_LOCK:
        ...
        python_path = str(PYTHON if PYTHON.exists() else Path(os.sys.executable))
        ARTIFACTS.mkdir(parents=True, exist_ok=True)
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            if query.get("offline") == ["1"]:
                return self._send(INDEX_HTML_OFFLINE)
        ...
        if path == "/app.js":
            return self._send(APP_JS, "application/javascript; charset=utf-8")
        if path == "/status":
