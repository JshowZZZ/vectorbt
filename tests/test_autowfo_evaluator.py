...
def test_evaluator_applies_overlay_filters_to_main_pf(monkeypatch):
    idx = pd.date_range("2026-01-01", periods=4, freq="h")
    symbols = ["ETH/USDT"]
    true_mask = pd.Series(True, index=idx)
    trade_close = pd.DataFrame({"ETH/USDT": [100.0, 101.0, 102.0, 103.0]}, index=idx)
    trade_mom = pd.DataFrame({"ETH/USDT": [0.2, 0.2, 0.2, 0.2]}, index=idx)
    overlay_long = pd.DataFrame({"ETH/USDT": [True, False, True, False]}, index=idx)
    overlay_short = pd.DataFrame({"ETH/USDT": [False, False, False, False]}, index=idx)
    ...
    ctx = {
        ...
        "htf_trend_filters": {
            "htf_trend:8h:10": {
                "long": overlay_long["ETH/USDT"],
                "short": overlay_short["ETH/USDT"],
            }
        },
    }
    runtime = {
    ...
    task = {
        ...
        "sl_stop": 0.006,
        "max_hold": 2,
        "filter_name": "none|htf_trend:8h:10",
        "filter_spec": {"kind": "htf_trend", "timeframe": "8h", "window": 10},
        "indicator_list": "",
    }
    ...
    def _fake_run_pf(*args, **kwargs):
        if "long_filter" in kwargs and "short_filter" in kwargs and "captured_main" not in captured:
            captured["captured_main"] = True
            captured["long_filter"] = kwargs["long_filter"].copy()
            captured["short_filter"] = kwargs["short_filter"].copy()
        return object()
    ...
    pd.testing.assert_frame_equal(captured["long_filter"], overlay_long)
...
def test_evaluator_state_trigger_mode_uses_trigger_entries_and_state_reversal_exits(monkeypatch):
    ...
    task = {
        ...
        "indicator_combo": ("state_a", "trigger_a"),
        "state_indicator_combo": ("state_a",),
        "trigger_indicator_combo": ("trigger_a",),
        ...
        "sl_stop": 0.006,
        "max_hold": 2,
        "filter_name": "state_trigger_pair",
        "indicator_list": "state_a,trigger_a",
    }

    monkeypatch.setattr(
        autowfo_evaluator.autowfo_engine,
        "_resolve_regime_signals",
        lambda **kwargs: (true_mask, false_mask, None, None),
    )
    monkeypatch.setattr(
        autowfo_evaluator.autowfo_engine,
        "_build_trade_mom_filters",
    ...
    state_long = pd.Series([True, True, False, False], index=idx)
    trigger_long = pd.Series([False, True, True, False], index=idx)

    def _fake_apply_indicator_combo(long_regime, short_regime, combo_keys, combo_params, combo_ctx):
        if tuple(combo_keys) == ("state_a",):
            return state_long, false_mask, combo_params
        if tuple(combo_keys) == ("trigger_a",):
            return trigger_long, false_mask, combo_params
        return long_regime, short_regime, combo_params
    ...
    def _fake_run_pf(close, long_signal, short_signal, *_args, **kwargs):
        captured["long_signal"] = long_signal.copy()
        captured["short_signal"] = short_signal.copy()
        captured["long_exits"] = kwargs["long_exits"].copy()
        captured["short_exits"] = kwargs["short_exits"].copy()
        return object()
    ...
    expected_long_entries = state_long & trigger_long
    expected_long_exits = ~state_long
    pd.testing.assert_series_equal(captured["long_signal"], expected_long_entries)
    pd.testing.assert_series_equal(captured["short_signal"], false_mask)
    pd.testing.assert_series_equal(captured["long_exits"], expected_long_exits)
    pd.testing.assert_series_equal(captured["short_exits"], pd.Series(True, index=idx))
