...
def test_cli_pilot_build_clue_map_writes_indicator_ranking(tmp_path):
    artifacts = tmp_path / "artifacts" / "runs"
    main_root = artifacts / "run-main"
    sens_root = artifacts / "run-sens"
    for root in [main_root, sens_root]:
    ...
    pd.DataFrame(
        [
            ...
            },
            {
                ...
                "sl_stop": 1.0,
                "max_hold": 4,
                "oos_avg_total_return_pct": 0.14,
                "oos_avg_total_trades": 1.0,
                "oos_sharpe_like": 1.6,
            },
        ]
    ).to_csv(main_root / "results" / "param_sweep_combo_summary.csv", index=False)
    pd.DataFrame(
        [
            {
                "timeframe": "2h",
    ...
    ).to_csv(sens_root / "results" / "param_sweep_combo_summary.csv", index=False)
    pd.DataFrame(
        [
            {
                "timeframe": "2h",
    ...
    (main_root / "metadata" / "run_metadata_run-main.json").write_text(
        json.dumps({"timeframe_diagnostics": [{"timeframe": "2h", "data_days": 180, "realized_shared_days": 180}]}),
        encoding="utf-8",
    )
    (sens_root / "metadata" / "run_metadata_run-sens.json").write_text(
        json.dumps({"timeframe_diagnostics": [{"timeframe": "2h", "data_days": 180, "realized_shared_days": 180}]}),
    ...
    out_json = tmp_path / "artifacts" / "clue_map.json"
    code = cli.main(
        [
    ...
    assert code == 0
    payload = json.loads(out_json.read_text(encoding="utf-8"))
    assert payload["summary"]["indicator_count"] == 2
